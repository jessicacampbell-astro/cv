%----------------------------------------------------------------------------------------
%	PACKAGES AND OTHER DOCUMENT CONFIGURATIONS
%----------------------------------------------------------------------------------------

\documentclass[10pt]{res} % Use the res.cls style, the font size can be changed to 11pt or 12pt here
\usepackage[top=0.5in, bottom=0.5in, left=1in, right=1in]{geometry}
\usepackage{helvet} % Default font is the helvetica postscript fontlX
\usepackage[breaklinks,colorlinks,citecolor=blue,urlcolor=blue,linkcolor=blue,anchorcolor=blue,backref=section]{hyperref}
\usepackage{tabularx}
\newsectionwidth{0pt} % Stops section indenting
\usepackage{graphicx}
\usepackage{enumitem}

\begin{document}

%----------------------------------------------------------------------------------------
%	YOUR NAME AND ADDRESS(ES) SECTION
%----------------------------------------------------------------------------------------

\hfill{\textit{Last modified \today}}

\name{\huge Jessica L. Campbell \href{https://orcid.org/0000-0002-2511-5256}{\includegraphics[scale=0.1]{orcid.png}}\\ 
\Large ~~~~~~Curriculum Vita\'e\\ \\} % Your name at the top

\address{
		\small{David A. Dunlap Department of Astronomy \& Astrophysics} \\
		\small{University of Toronto} \\ 
		\small{50 St. George Street, M5S 3H4} \\
		\small{Toronto, Ontario, CA}  \\ 
		\small{AB 223}
		}

\address{
		\small{Website: \href{https://www.astro.utoronto.ca/~campbell}{astro.utoronto.ca/{{\raise.17ex\hbox{$\scriptstyle\sim$}}}campbell}} \\ 
		\small{GitHub: \href{https://github.com/astrosica}{github.com/astrosica}} \\ 
		\small{LinkedIn: \href{https://www.linkedin.com/in/astrosica}{linkedin.com/in/astrosica}} \\ 
		\small{Email: \href{mailto:campbell@astro.utoronto.ca}{campbell@astro.utoronto.ca}}
		%\small{Office: +1 (416) 978-3148} 
		}

%----------------------------------------------------------------------------------------

\begin{resume}

%----------------------------------------------------------------------------------------
%	RESEARCH INTERESTS
%----------------------------------------------------------------------------------------

\section{\Large Research Interests}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text

Galactic magnetism; multi-phase interstellar medium; radio polarization; dust extinction; star formation

%----------------------------------------------------------------------------------------
%	EDUCATION
%----------------------------------------------------------------------------------------

\section{\Large Education}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text

{\bf Ph.D.}, Astronomy \& Astrophysics, DADDAA \& Dunlap, UofT \hfill Sept 2016 -- Jan 2023 \\
Preliminary thesis entitled ``\textit{Polarimetry and 21 cm HI as a Probe of Galactic Magnetism in a Multi-phase Interstellar Medium}'' supervised by Bryan Gaensler and Susan Clark

{\bf H.B.Sc.}, Astronomy \& Physics, UofT \hfill Sept 2011 -- June 2016 \\ 
Undergraduate thesis entitled ``\textit{Using High-Resolution JCMT Imaging to Complement Herschel Observations in Regions of Massive Star Formation}'' supervised by Peter Martin
 
%----------------------------------------------------------------------------------------
%	HONOURS AND AWARDS
%----------------------------------------------------------------------------------------

\section{\Large Honours and Awards}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text

\begin{table}[h!]
\begin{tabularx}{\textwidth}{lX}
% CASCA poster (CASCA board)
2021 & \textbf{CASCA Poster Prize}, CASCA Board \\
% OGS
2021 & \textbf{Ontario Graduate Scholarship} (\$15,000), DADDAA, UofT  \\
% Dundies
2020 & \textbf{`You Lead, We Follow' Award}, Dunlap, UofT \\
% CASCA poster (CASCA board)
2020 & \textbf{CASCA Poster Prize}, CASCA Board \\
% CASCA poster (GSC)
2020 & \textbf{CASCA Poster Prize}, CASCA GSC \\
% Reinhardt fund
2019 & \textbf{Reinhardt Fund Award}, DADDAA, UofT \\
% Dundies
2018 & \textbf{`Above and Beyond' Award}, Dunlap, UofT \\
% UofT Fellowship
2018 -- 2019 & \textbf{University of Toronto Fellowship} (\$11,742), UofT \\
% UofT Fellowship
2017 -- 2018 & \textbf{University of Toronto Fellowship} (\$11,742), UofT \\
% UofT Fellowship
2016 -- 2017 & \textbf{University of Toronto Fellowship} (\$15,855), UofT \\
% DAA Fellowship
2016 -- 2017 & \textbf{DADDAA Fellowship} (\$8,891), DADDAA, UofT \\
% CITA summer research
2016 & \textbf{CITA Undergraduate Summer Research Award} (\$8,400), CITA, UofT \\
% Reinhardt fund
2016 & \textbf{Reinhardt Fund Award}, DADDAA, UofT \\
% Conference grant
2016 & \textbf{Conference Travel Grant}, Dunlap \& DADDAA, UofT \\
% LEAPS summer research
2015 & \textbf{Leiden/ESA Astrophysics Program for Summer Students Fellowship}, Leiden Observatory, Leiden University \\
% SURP
2014 & \textbf{Summer Undergraduate Research Program Award} (\$8,000), Dunlap, UofT \\
% SURP
2013 & \textbf{Summer Undergraduate Research Program Award} (\$10,000), Dunlap, UofT
\end{tabularx}
\end{table}

%----------------------------------------------------------------------------------------
%	REFEREED PUBLICATIONS
%----------------------------------------------------------------------------------------

\section{\Large Refereed Publications}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-15pt} % Gap between title and text

\begin{enumerate}[{start=3,label=\arabic*.\addtocounter{enumi}{-2}}]
\itemsep-1em 
	\item \textbf{J. L. Campbell}, S. E. Clark, B. M. Gaensler, A. Marchal, et al., ``\textit{A Comparison of Multi-phase Magnetic Field Tracers in a High-Galactic Latitude Region of the Filamentary Interstellar Medium,}" 2021, to appear in ApJ (34 pages, submitted) \\
	%\item \textbf{J. L. Campbell}, A. Roy, P. G. Martin, M. Rahman, S. Song, \& L. Einstein, ``\textit{On the Bright Infrared Stars in the W3 Giant Molecular Cloud,}" 2021, to appear in ApJ (in prep) \\
	\item A. J. M. Thomson, T. L. Landecker, N. M. McClure-Griffiths, et al. including \textbf{J. L. Campbell}, ``\textit{The Global Magneto-Ionic Medium Survey (GMIMS): The brightest polarized region in the Southern sky at 75 cm and its implications for Radio Loop II,}" 2021, to appear in MNRAS (submitted) \\
	\item \textbf{J. L. Campbell}, R. K. Friesen, P. G. Martin, P. Caselli, J. Kauffmann, \& J. E. Pineda, ``\textit{Contraction Signatures Toward Dense Cores in the Perseus Molecular Cloud,}" 2016, \href{https://ui.adsabs.harvard.edu/abs/2016ApJ...819..143C/abstract}{ApJ, 819, 143C} (17 pages)
\end{enumerate}

%----------------------------------------------------------------------------------------
%	SCIENTIFIC PRESENTATIONS
%----------------------------------------------------------------------------------------

\section{\Large Scientific Presentations}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text

\begin{table}[h!]
\begin{tabularx}{\textwidth}{lX}
% Invited talks
\multicolumn{2}{l}{ \rule{0pt}{3ex} \large \hspace{-12pt} Invited Talks \dotfill \rule[-1.2ex]{0pt}{0pt}} \\
% DRAO Seminar Series
July 2021 & \textbf{DRAO Seminar Series}; Dominion Radio Astrophysical Observatory (\textit{virtual}) \\
% AAS 238
June 2021 & \textbf{AAS 238}; Arecibo Observatory - Legacy and Future Splinter Session (\textit{virtual}) \\
% Presentations
\multicolumn{2}{l}{ \rule{0pt}{3ex} \large \hspace{-12pt} Contributed Talks and Posters \dotfill \rule[-1.2ex]{0pt}{0pt}} \\
% CASCA 2021
May 2021 & \textbf{CASCA} (poster); NRC Herzberg Astronomy and Astrophysics Research Center (\textit{virtual}) \\
% CASCA 2020
May 2020 & \textbf{CASCA} (poster); York University (\textit{virtual}) \\
% New Perspectives on Galactic Magnetism
June 2019 & \textbf{New Perspectives on Galactic Magnetism}; University of Newcastle upon Tyne, England \\
% Big Apple Magnetic Fields
Jan. 2019 & \textbf{Big Apple Magnetic Fields}; Flatiron Center for Computational Astrophysics, NY \\
% MW in Age of Gaia
Nov. 2018 & \textbf{Milky Way in the Age of GAIA}; Institut d'Astrophysique Spatiale, France \\
% CCUWiP
2016 & \textbf{CCUWiP} (poster); Dalhousie University, NS \\
% Workshops
\multicolumn{2}{l}{ \rule{0pt}{3ex} \large \hspace{-12pt} Workshops \dotfill \rule[-1.2ex]{0pt}{0pt}}  \\
% GMIMS Science Workshop
Oct. 2017 & \textbf{GMIMS Science Workshop}; Dominion Radio Astrophysical Observatory, BC \\
% PyAstro
May 2017 & \textbf{Python in Astronomy Conference}; Lorentz Center, Leiden University 
\end{tabularx}
\end{table}

%----------------------------------------------------------------------------------------
%	TEACHING AND MENTORSHIP
%----------------------------------------------------------------------------------------

\section{\Large Teaching and Mentorship}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text
\begin{table}[h!]
\begin{tabularx}{\textwidth}{ @{} p{6.5em} X @{} }
%
% STUDENT SUPERVISION
%
\multicolumn{2}{l}{ \rule{0pt}{3ex} \large \hspace{-12pt} Student Supervision \dotfill \rule[-1.2ex]{0pt}{0pt}} \\
% 2020-2021
2020 -- 2021 & \textbf{Parampreet Singh}; co-supervised with Jennifer West, undergraduate thesis (AST425Y) \\
%
% STUDENT MENTORSHIP
%
\multicolumn{2}{l}{ \rule{0pt}{3ex} \large \hspace{-12pt} Student Mentorship \dotfill \rule[-1.2ex]{0pt}{0pt}} \\
% 2018-2019
2018 -- 2019 & \textbf{James Lane}; incoming PhD student, UofT \\
% 2017-2018
2017 -- 2018 & \textbf{Colleen Gilhuly}; incoming PhD student, UofT \\
                     & \textbf{Victor Chan}; incoming PhD student, UofT \\
%
% TEACHING ASSISTANTSHIPS
%
\multicolumn{2}{l}{ \rule{0pt}{3ex} \large \hspace{-12pt} Teaching Assistantships \dotfill \rule[-1.2ex]{0pt}{0pt}} \\
% GRAD 5th YEAR
Summer 2021 & \textbf{Stars and Galaxies} (AST201) \\
Winter 2021    & \textbf{Galaxies and Cosmology} (AST222) \\
Fall 2020         & \textbf{Practical Astronomy} (AST325) \\
% GRAD 4th YEAR
Winter 2020    & \textbf{Galaxies and Cosmology} (AST222) \\
Fall 2019         & \textbf{Stars and Planets} (AST221) \\
% GRAD 3rd YEAR
Summer 2019 & \textbf{Life on Other Worlds} (AST251) \\
Winter 2019    & \textbf{Life on Other Worlds} (AST251) \\
Fall 2018         & \textbf{Great Moments in Astronomy} (AST210) \\
% GRAD 2nd YEAR
Winter 2018    & \textbf{Life on Other Worlds} (AST251) \\
Fall 2017         & \textbf{Great Moments in Astronomy} (AST210) \\
% GRAD 1st YEAR
Winter 2017    & \textbf{Stars and Galaxies} (AST201) \\
Fall 2016         & \textbf{Stars and Galaxies} (AST101) \\
% UNDERGRAD 5th YEAR
Winter 2016    & \textbf{The Sun and its Neighbours} (AST201) \\
                        & \textbf{Great Astronomical Issues} (PMU199) \\
% UNDERGRAD 4th YEAR
Winter 2015    & \textbf{Stars and Galaxies} (AST201)
\end{tabularx}
\end{table}

%----------------------------------------------------------------------------------------
%	RESEARCH EXPERIENCE 
%----------------------------------------------------------------------------------------

\section{\Large Research Experience}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text

\begin{table}[h!]
%\begin{tabularx}{\textwidth}{lX}
\begin{tabularx}{\textwidth}{ @{} p{6.5em} X @{} }
% Graduate
\multicolumn{2}{l}{ \rule{0pt}{3ex} \large \hspace{-12pt} Graduate \dotfill \rule[-1.2ex]{0pt}{0pt}} \\
% PhD Thesis
2017 -- 2023 & \textbf{Ph.D. Thesis}, DADDAA \& Dunlap, UofT \\
                     & Preliminary thesis entitled ``\textit{Polarimetry and 21 cm HI as a Probe of Galactic Magnetism in a Multi-phase Interstellar Medium}'' supervised by Bryan Gaensler and Susan Clark \\
% AST1500 
2017 & \textbf{Graduate Research Course}, DADDAA \& Dunlap, UofT \\
         & ``\textit{A Magneto-ionic Radio Polarization Study of the Orion-Eridanus Superbubble Region, Loop III and the Intermediate Velocity Arch}'' supervised by Bryan Gaensler \\
% AST1501
2016 -- 2017 & \textbf{Graduate Research Course}, DADDAA \& Dunlap, UofT  \\
                     & ``\textit{Interstellar Extinction Towards OB Stars in the Molecular Interstellar Medium of W3}'' supervised by Peter Martin
\end{tabularx}
\end{table}

\begin{table}[h!]
%\begin{tabularx}{\textwidth}{lX}
\begin{tabularx}{\textwidth}{ @{} p{6.5em} X @{} }
% Undergraduate
\multicolumn{2}{l}{ \rule{0pt}{3ex} \large \hspace{-12pt} Undergraduate \dotfill \rule[-1.2ex]{0pt}{0pt}} \\ 
% SURP
2016 & \textbf{Summer Undergraduate Researcher}, CITA, UofT \\
         & ``\textit{Bright Infrared Stars in the W3 Giant Molecular Cloud}'' supervised by Peter Martin \\
% LEAPS
2015 & \textbf{Leiden/ESA Astrophysics Program for Summer Students}, Leiden University \\
         & ``\textit{Astrochemical Conditions of the Low-Mass Protostar IRAS 16293-2422 using ALMA}'' supervised by Mihkel Kama and Magnus Persson \\
% AST425
2014 -- 2015 & \textbf{Undergraduate Research Course}, DADDAA, UofT \\
                     & ``\textit{Using High-Resolution JCMT Imaging to Complement Herschel Observations in Regions of Massive Star Formation}'' supervised by Peter Martin \\
% SURP
2014 & \textbf{Summer Undergraduate Research Program}, Dunlap, UofT \\
         & ``\textit{Connecting Local Galaxies to Damped Lyman Alpha Systems via 21-cm}'' supervised by Brian Cherinka \\
% CTA395
2013 -- 2014 &\textbf{Undergraduate Research Course}, CITA, UofT \\
                     & ``\textit{Molecular Line Tracers of Kinematics in Star-Forming Regions}'' supervised by Rachel Friesen and Peter Martin \\
% SURP
2013 & \textbf{Summer Undergraduate Research Program}, Dunlap, UofT \\
         & ``\textit{Contraction Signatures Toward Dense Cores in the Perseus Molecular Cloud}'' supervised by Rachel Friesen
\end{tabularx}
\end{table}

%----------------------------------------------------------------------------------------
%	ACADEMIC SERVICE
%----------------------------------------------------------------------------------------

\newpage
\section{\Large Academic Service}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text

\begin{table}[h!]
\begin{tabularx}{\textwidth}{lX}
% ASU Panel Discussion
2020 & \textbf{Graduate Panel Discussion}; ASU, UofT (\textit{virtual}) \\
% SURP Panel Discussion
2020 & \textbf{SURP Graduate Student Panel Discussion}; DADDAA, UofT (\textit{virtual}) \\
% ASU Panel Discussion
2020 & \textbf{Graduate Student Panel Discussion}; ASU, UofT \\
% TMT Statement Committee
2019 -- 2020 & \textbf{TMT Statement Committee}; GASA, UofT (elected) \\
% PRISM
2019 -- 2020 & \textbf{People Representing Intersectional Spectral Minorities}; GASA, UofT (elected) \\
% DMC Student Rep
2018 -- 2020 & \textbf{Dunlap Management Committee (student rep)}; DADDAA, UofT (elected) \\
% Values Statement Committee Member
2017 -- 2018 & \textbf{Values Statement Committee}; DADDAA, UofT \\
% Indigenous Astronomy Workshop Committee Member
2017 -- 2018 & \textbf{Indigenous Astronomy  Workshop Committee}; DADDAA, UofT \\
% GASA CUPE Rep.
2016 --  2018 & \textbf{CUPE Local 3902 Unit 1 Representative}; GASA, UofT (elected) \\
% DDC
2015 --  2018 & \textbf{Dunlap Diversity Committee (DDC)}; Dunlap, UofT \\
% Dunlap Summer School
2017 & \textbf{Dunlap Summer School Admissions Committee}; Dunlap, UofT \\
% GASA Mediation Committee
2017 -- 2018 & \textbf{Mediation Committee}; GASA, UofT (elected) \\
% GASA Course Committee
2017 -- 2018 & \textbf{Graduate Course Committee}; GASA, UofT (elected) \\
% GASA Mental Health Committee
2017 -- 2018 & \textbf{Mental Health Committee}; GASA, UofT (elected) \\
% Dunlap Associate
2016 --  2017 & \textbf{Dunlap Associate}; Dunlap, UofT
\end{tabularx}
\end{table}

%----------------------------------------------------------------------------------------
%	PROFESSIONAL DEVELOPMENT
%----------------------------------------------------------------------------------------

%\section{\Large Professional Development}
%\vspace{-5pt} % Gap between title and line
%\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
%\vspace{-20pt} % Gap between title and text

%\begin{table}[h!]
%\begin{tabularx}{\textwidth}{lX}
% Dunlap Retreat 
%2020 & \textbf{Dunlap Annual Retreat}; Dunlap, UofT \\
% IAR workshop
%2020 & \textbf{Developing Effective Mentoring Practices in the Academy}; Dunlap, UofT \\
% IAR workshop
%2019 & \textbf{Identify, Assist, Refer Workshop}; DADDAA, UofT \\
% TATP: Teaching Assistants' Training Program (TATP)
%2015 -- 2020  & \textbf{Teaching Assistants' Training Program (TATP)}; UofT \\
% ALMA Community Day
%2017 & \textbf{ALMA Community Day Event}; NRAO \& DADDAA, UofT \\
% Dunlap Retreat 
%2017 & \textbf{Dunlap Annual Retreat}; Dunlap, UofT \\
% Dunlap Retreat 
%2016 & \textbf{Dunlap Annual Retreat}; Dunlap, UofT \\
%  CUPE Steward's Training
%2016 & \textbf{CUPE Unit 1 Local 3902 Stewards' Training}; CUPE, UofT
%\end{tabularx}
%\end{table}

%----------------------------------------------------------------------------------------
%	PUBLIC OUTREACH
%----------------------------------------------------------------------------------------

\section{\Large Outreach}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text

\begin{table}[h!]
\begin{tabularx}{\textwidth}{lX}
% Grad Hlighlights
2020 & \textbf{GSC's Grad Highlights}, CASCA (\href{https://casca.ca/?page_id=14667}{\textbf{link to article}}) \\
% AstroTours
2013 -- 2017 & \textbf{AstroTours Volunteer}, UofT \\
% ASX
2015 -- 2016 & \textbf{Director of Outreach}, Astronomy and Space Exploration Society, UofT \\
% Fall Campus Day
2015 & \textbf{Fall Campus Day}, UofT \\
% ASX
2013 -- 2014 & \textbf{Director of Outreach}, Astronomy and Space Exploration Society, UofT \\
% Science Rendezvous
2015 -- 2014 & \textbf{Science Rendezvous Solar Telescope Operator}, UofT \\
% The Star Spot
2013 -- 2014 & \textbf{Current in Space News Producer}, The Star Spot podcast \\
% Sidewalk Astronomy
2013 & \textbf{Sidewalk Astronomy Solar Telescope Operator}, UofT
\end{tabularx}
\end{table}

%----------------------------------------------------------------------------------------
%	MEDIA COVERAGE
%----------------------------------------------------------------------------------------

\section{\Large Media Coverage}
\vspace{-5pt} % Gap between title and line
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
\vspace{-20pt} % Gap between title and text

\begin{table}[h!]
\begin{tabularx}{\textwidth}{lX}
% Road trip
2017 & \textbf{Road Trip! UofT Students and Faculty Chase Total Solar Eclipse}; \textit{UofT News}, UofT (\href{https://www.utoronto.ca/news/road-trip-u-t-students-and-faculty-chase-total-solar-eclipse}{\textbf{link to article}})
\end{tabularx}
\end{table}



\end{resume} 
\end{document}